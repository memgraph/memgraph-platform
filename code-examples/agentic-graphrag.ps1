...
#
# Requirements:
#   - Docker Desktop  https://docs.docker.com/desktop/install/windows-install/  (required)
...
# (checked explicitly below), not stderr, decide success. Keep PowerShell from
...
$Work      = Join-Path $ScriptDir '.agentic-graphrag-work'
...
for ($start = 0; $start -lt $lines.Count; $start += $batch) {
    ...
    Invoke-MgQuiet (($lines[$start..$end]) -join "`n") | Out-Null
}
...
one of the three pipelines above), set an OpenAI key and re-run:
  ...
  .\agentic-graphrag.ps1

Tear everything down when you are done:
  .\agentic-graphrag.ps1 clean
"@
    exit 0
}
